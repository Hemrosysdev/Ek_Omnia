...
#define MCU_CONFIG_FILE        "/config/McuConfig.json"
...
McuDriver::McuDriver( QObject * parent )
    ...
    , m_strSwVersion( "" )
{
    MainStatemachine::ensureExistingDirectory( MainStatemachine::rootPath() + MCU_CONFIG_FILE );
...
bool McuDriver::open(const QString &strPortName)
{
    ...
    if ( !isOpen() )
    {
        qDebug() << "McuDriver::open() " << strPortName;

        bSuccess = m_pUpppMcu->open( strPortName );

        if ( !bSuccess )
        {
            qDebug() << "McuDriver::open() failed " << m_pUpppMcu->errorString();
        }
    }
...
bool McuDriver::startSwUpdate()
{
    ...
    if ( isMcuAlive() )
    {
        ...
        {
            QFile  jsonFile( MainStatemachine::rootPath() + MCU_FIRMWARE_PATH "/" MCU_FIRMWARE_CONFIG_JSON );
            ...
            if ( jsonFile.open( QIODevice::ReadOnly ) )
            {
                ...
                QString strFirmwareFile = doc.object()["FirmwareFile"].toString();
            ...
            {
                qCritical() << "startSwUpdate: can't open JSON config file" << jsonFile.fileName();
            }
        }
        else
        {
...
bool McuDriver::isSystemStatusSafeLock() const
{
    return m_bSystemStatusSafeLock;
}
                                     ...
                                     const bool bRotorLocked )
{
    ...
    setSystemStatusSafeLock( bSafeLock );
...
{
    bool bAlive = m_bStatusAcknowledged && m_bMcuStatusValid && m_bSystemStatusHmiAlive;
...
void McuDriver::setSystemStatusHmiAlive( const bool bAlive )
{
    if ( m_bSystemStatusHmiAlive != bAlive )
    {
        ...
        emit systemStatusHmiAliveChanged();
    }
}
...
void McuDriver::setSystemStatusMotorRunning( const bool bRunning )
{
    if ( m_bSystemStatusMotorRunning != bRunning )
    {
        ...
        emit systemStatusMotorRunningChanged();
    }
}
...
void McuDriver::setSystemStatusSimulation( const bool bSimulation )
{
    if ( m_bSystemStatusSimulation != bSimulation )
    {
        ...
        emit systemStatusSimulationChanged();
    }
}
...
void McuDriver::setSystemStatusSafeLock( const bool bSafeLock )
{
    if ( m_bSystemStatusSafeLock != bSafeLock )
    {
        m_bSystemStatusSafeLock = bSafeLock;
        emit systemStatusSafeLockChanged();
    }
}
...
void McuDriver::setSystemStatusFanRunning( const bool bRunning )
{
    if ( m_bSystemStatusFanRunning != bRunning )
    {
        ...
        emit systemStatusFanRunningChanged();
    }
}
...
void McuDriver::readMcuConfig()
{
    QFile file( MainStatemachine::rootPath() + MCU_CONFIG_FILE );

    m_u32ConfigMaxMotorSpeed     = 1700;
    m_u32ConfigNominalMotorSpeed = 1500;
    m_u32ConfigAccelerationTime  = 200;
    m_u32ConfigDecelerationTime  = 200;

    if ( file.open( QIODevice::ReadOnly ) )
    {
        QByteArray strJson = file.readAll();
        file.close();

        QJsonDocument doc( QJsonDocument::fromJson( strJson ) );

        if ( doc.object().contains( "FileVersion" ) && doc.object()["FileVersion"].isString() )
        {
            if ( doc.object()["FileVersion"].toString() == "1" )
            {
                m_u32ConfigMaxMotorSpeed     = static_cast<quint32>( doc.object()["MaxMotorSpeed"].toInt() );
                m_u32ConfigNominalMotorSpeed = static_cast<quint32>( doc.object()["NominalMotorSpeed"].toInt() );
                m_u32ConfigAccelerationTime  = static_cast<quint32>( doc.object()["AccelerationTime"].toInt() );
                m_u32ConfigDecelerationTime  = static_cast<quint32>( doc.object()["DecelerationTime"].toInt() );
            }
            else
            {
                qCritical() << "readMcuConfig(): incompatible version " << doc.object()["FileVersion"].isString() << " in file <" << file.fileName() << ">";
            }
        }
        else
        {
            qCritical() << "readMcuConfig(): no version in file <" << file.fileName() << ">";
        }
    }
    else
    {
        qCritical() << "readMcuConfig(): failed to read file <" << file.fileName() << ">";
    }
}
...
void McuDriver::writeMcuHwSwRef()
{
    if ( isProductIdValid() )
    {
        QFile file( MainStatemachine::rootPath() + MCU_HW_SW_REF_FILE );
        ...
        {
            qDebug() << "readMcuConfig(): failed to read file <" << file.fileName() << ">";
        }
    }
}
