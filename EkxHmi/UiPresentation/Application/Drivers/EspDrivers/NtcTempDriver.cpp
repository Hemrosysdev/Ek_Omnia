...
NtcTempDriver::NtcTempDriver( QQmlEngine* pEngine,
                              MainStatemachine * pMainStatemachine )
    : QObject( pMainStatemachine )
    , m_pMainStatemachine( pMainStatemachine )
{
    pEngine->rootContext()->setContextProperty( "ntcTempDriver" , this);
}
...
void NtcTempDriver::create()
{
    connect( m_pMainStatemachine->settingsStatemachine()->settingsSerializer(),
             &SettingsSerializer::showTempWarningChanged,
             this,
             &NtcTempDriver::updateOvertempWarning );
...
void NtcTempDriver::connectInterface( EspAdcIn *ntcSensorIn )
{
    if ( m_pNtcSensorIn != ntcSensorIn )
    {
        disconnectInterface();
        m_pNtcSensorIn = ntcSensorIn;

        if ( m_pNtcSensorIn )
        {
            connect( m_pNtcSensorIn, &EspDataInterface::dataChanged, this, &NtcTempDriver::processNtcSensorDataChanged );
            connect( m_pNtcSensorIn, &EspDataInterface::validChanged, this, &NtcTempDriver::processNtcSensorValidChanged );

            setCurrentTemp( adcToTemperature( m_pNtcSensorIn->voltage() ) );
            setCurrentTempValid( m_pNtcSensorIn->isValid() );
        }
    }
}
...
void NtcTempDriver::disconnectInterface( )
{
    setCurrentTemp( 0 );
    setCurrentTempValid( false );
    setOvertempWarningActive( false );

    m_pNtcSensorIn = nullptr;
}
...
void NtcTempDriver::processNtcSensorDataChanged( EspDataInterface * pData )
{
    EspAdcIn * ntcSensorIn = dynamic_cast<EspAdcIn*>( pData );
    if ( ntcSensorIn )
    {
        setCurrentTemp( adcToTemperature( ntcSensorIn->voltage() ) );
    }
}
...
void NtcTempDriver::processNtcSensorValidChanged( EspDataInterface * pData )
{
    EspAdcIn * ntcSensorIn = dynamic_cast<EspAdcIn*>( pData );
    if ( ntcSensorIn )
    {
        setCurrentTempValid( ntcSensorIn->isValid() );
    }
}
...
void NtcTempDriver::updateOvertempWarning()
{
    setOvertempWarningActive( m_bCurrentTempValid
                              && ( m_pMainStatemachine->settingsStatemachine()->settingsSerializer()->getShowTempWarning() )
                              && ( m_nCurrentTemp >= ( m_pMainStatemachine->settingsStatemachine()->settingsSerializer()->getTempThresValueX100() / 100.0 ) ) );
}
