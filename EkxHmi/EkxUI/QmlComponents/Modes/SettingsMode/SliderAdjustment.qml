...
Item
{
    ...
    function calcCurrentValue( mousePos )
    {
        ...
        currentValueX100 = Math.floor( currentValueX100 / ( step * 100 ) ) * ( step * 100 );
    ...
    {
        if (entryType === SettingsStateEnum.SETTING_CUSTOMIZE_BRIGHTNESS)
            displayController.brightness = settingsSerializer.displayBrightness;
    }
    ...
    {
        var tempValue = div100RoundClip( currentValueX100 );
        var strContext;
        switch ( entryType )
        {
        case SettingsStateEnum.SETTING_CUSTOMIZE_BRIGHTNESS:
            strContext = settingsSerializer.displayBrightness + "->" + tempValue;
            settingsSerializer.displayBrightness = tempValue;
            settingsSerializer.saveSettings();

            var a = EkxSqliteTypes.SqliteEventType_CHANGE_BRIGHTNESS;
            var b = EkxSqliteTypes.SqliteNotificationType_UNKNOWN;
        ...
        case SettingsStateEnum.SETTING_SERVICE_MCU_CONFIG_MAX_MOTOR_SPEED:
            strContext = settingsSerializer.mcuMaxMotorSpeed + "->" + tempValue;
            settingsSerializer.mcuMaxMotorSpeed = tempValue; // no rounding!
            settingsSerializer.saveSettings();
            settingsSerializer.raiseMcuConfigChanged();

            sqliteInterface.addEvent( EkxSqliteTypes.SqliteEventType_CHANGE_MCU_MAX_MOTOR_SPEED,
                                     strContext );
            break;
        case SettingsStateEnum.SETTING_SERVICE_MCU_CONFIG_NOMINAL_MOTOR_SPEED:
            strContext = settingsSerializer.mcuNominalMotorSpeed + "->" + tempValue;
            settingsSerializer.mcuNominalMotorSpeed = tempValue; // no rounding!
            settingsSerializer.saveSettings();
            settingsSerializer.raiseMcuConfigChanged();

            sqliteInterface.addEvent( EkxSqliteTypes.SqliteEventType_CHANGE_MCU_NOMINAL_MOTOR_SPEED,
                                     strContext );
            break;
        case SettingsStateEnum.SETTING_SERVICE_MCU_CONFIG_ACCELERATION_TIME:
            strContext = settingsSerializer.mcuAccelerationTime + "->" + tempValue;
            settingsSerializer.mcuAccelerationTime = tempValue; // no rounding!
            settingsSerializer.saveSettings();
            settingsSerializer.raiseMcuConfigChanged();

            sqliteInterface.addEvent( EkxSqliteTypes.SqliteEventType_CHANGE_MCU_ACCELERATION_TIME,
                                     strContext );
            break;
        case SettingsStateEnum.SETTING_SERVICE_MCU_CONFIG_DECELERATION_TIME:
            strContext = settingsSerializer.mcuDecelerationTime + "->" + tempValue;
            settingsSerializer.mcuDecelerationTime = tempValue; // no rounding!
            settingsSerializer.saveSettings();
            settingsSerializer.raiseMcuConfigChanged();

            sqliteInterface.addEvent( EkxSqliteTypes.SqliteEventType_CHANGE_MCU_DECELERATION_TIME,
                                     strContext );
            break;
        }
    }
    ...
    onCurrentValueX100Changed:
    {
        if ( entryType === SettingsStateEnum.SETTING_CUSTOMIZE_BRIGHTNESS )
        {
            displayController.brightness = div100RoundClip( currentValueX100 );
        }
    }
    ...
    Item
    {
        ...
        Rectangle
        {
            ...
            MouseArea
            { // Same drag behaviour like on idButtonIncrease/idButtonDecrease
                //anchors.fill: parent
                //onPressed: {
        ...
        Text
        {
            ...
            text:
            {
                if( entryType === SettingsStateEnum.SETTING_CUSTOMIZE_TEMP_THRESH)
        ...
        Rectangle
        {
            ...
            MouseArea
            {
                ...
                onClicked:
                {
                    currentValueX100 = currentValueX100 + ( step * 100 )
                    if ( currentValueX100 > maxValue*100 )
                    {
                        currentValueX100 = maxValue*100;
                    }
