...
void HmiTempDriver::create( MainStatemachine * pMainStatemachine )
{
...
void HmiTempDriver::processLogTimeout()
{
    ...
    if ( deviceFile.open( QIODevice::ReadOnly ) )
    {
        ...
        int nRawTemp = tempStr.toInt() * 165;
        nRealTemp  = ( nRawTemp >> 16 ) - 40;
        bReadValid = true;
    }

    // check if file exists to write the header
    bool bLogExists = logFile.exists();
    ...
    if ( logFile.open( QIODevice::WriteOnly | QIODevice::Append ) )
    {
        ...
        if ( !bLogExists )
        {
            stream << "Timestamp;Temperature;Valid" << QT_ENDL;
        }

        stream << currentDateTime.toString( "yyyy-MM-dd hh:mm:ss" ) << ";" << nRealTemp << ";" << bReadValid << QT_ENDL;
        logFile.flush();
        logFile.close();
    }
