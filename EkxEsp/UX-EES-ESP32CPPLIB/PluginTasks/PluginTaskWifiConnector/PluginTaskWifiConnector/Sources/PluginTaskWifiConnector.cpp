...
// uncomment this line for test purposes
//#define TX_POWER   10
                                         ...
                                         const bool bStaModeChanged )
{
    ...
    if ( m_bWifiOn
         && !m_bSleepOn )
    {
        if ( /*!m_bWifiStarted && */ ( nEspError == ESP_OK )
             && ( bModeChanged
                  || bApModeChanged
                  || bStaModeChanged ) )
        {
...
void PluginTaskWifiConnector::startWifiScan( void )
{
    if ( m_bWifiOn )
    {
        if ( m_nConfigWifiMode == PluginTaskWifiConnectorTypes::WifiMode::Ap )
        {
            ...
            // won't work
            if ( !m_strConfigStaSsid.empty()
                 || !m_strConfigStaPassword.empty() )
            {
                wifi_config_t wifi_config;
                std::memset( &wifi_config, 0, sizeof( wifi_config ) );
